...
n <- length(cavities)
mu <- mean(cavities)
se <- round(sqrt(var(cavities)*(N-n)/N), 3)
```
...
$$P\left(\left|\bar{y} - \mu\right| < B\right) \approx 0.95$$
and we know $B = 2\SE{\bar{y}} = 2\sqrt{\frac{s^2}{n}\left(\frac{N-n}{N}\right)}$ is a solution to this equation. 
...
As usual, the methods for population total follow from the methods for population mean. In this case the desired , call it now $B_\tau$, is simply divided by $N$ and used as in the previous formula with $B = \frac{B_tau}{N}$.
